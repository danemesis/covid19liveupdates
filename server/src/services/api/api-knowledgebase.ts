...
export function fetchKnowledgeMetainformation(): Promise<KnowledgebaseMeta> {
    return axios
        .get<KnowledgebaseMeta>(`${environments.KNOWLEDGEBASE_URL}/meta/all`, {
            headers: {
                'x-access-token': environments.KNOWLEDGEBASE_SECRET_KEY,
            },
        })
...
export function fetchAnswer(question: string): Promise<Array<Answer>> {
    return axios
        .get<ApiAnswer>(`${environments.KNOWLEDGEBASE_URL}/question`, {
            headers: {
                'x-access-token': environments.KNOWLEDGEBASE_SECRET_KEY,
            },
            params: { question },
        ...
        .then(
            (answers: Array<ApiAnswer>): Array<Answer> =>
                answers.map(
                    ({
                        category,
                        countries,
